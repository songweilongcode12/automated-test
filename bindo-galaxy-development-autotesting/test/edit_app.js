// 编辑app
async function EditApp( driver, webdriver, params) {
  const {
    By,
    until,
  } = webdriver;
  try {
    ...
    await driver.sleep(2000);
    const editdatas = `E${params.createUuid()}`;
...
// 编辑系统app
async function EditsystemApp( driver, webdriver, params) {
  const {
    By,
    until,
  } = webdriver;
  try {
    ...
    await driver.sleep(2000);
    const editdatas1 = `E${params.createUuid()}`;
    await driver.findElement(By.xpath("//input[@class='ant-input']")).clear();
